...
class datastore():
	...
	def create(self):
		if self.ttl:
			return main_ex.create(self.client, self.key, self.value, ttl = int(self.ttl), filepath = self.filepath)
		else:
			return main_ex.create(self.client, self.key, self.value, filepath = self.filepath)


	def read(self):
		return main_ex.read(self.client, self.key, filepath = self.filepath)


	def delete(self):
		return main_ex.delete(self.client, self.key, filepath = self.filepath)


	def reset(self):
		return main_ex.reset(self.client, filepath = self.filepath)


def datastore_get(operation_name, **kwargs):
...
if __name__ == "__main__": 
	...
	print(datastore_get(operation, client = client, key = key, value = value, ttl = ttl, filepath = filepath))
